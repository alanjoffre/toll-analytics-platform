...
IGNORE_BASENAMES = {"gerar_documentacao.py", "check_docs_sync.py",
...
def find_adrs_in_code():
    ...
    adrs = set()
    patterns = ["README.md", "PLANO_DO_PROJETO.md",
                "dbt_project.yml", "models/**/*.sql", "models/**/*.yml",
                "snapshots/*.sql", "macros/*.sql", "seeds/*.yml"]
    for pat in patterns:
...
def find_project_files():
    ...
    files = set()
    for pat in ["seeds/*.csv", "seeds/*.yml",
                "models/**/*.sql", "models/**/*.yml",
                "snapshots/*.sql", "macros/*.sql"]:
        for f in glob.glob(os.path.join(ROOT, pat), recursive=True):
...
def main():
    if not os.path.exists(DOCX):
        print(f"ERRO: Word não encontrado em {DOCX}. Rode: python docs/gerar_documentacao.py")
        return 1
    ...
    if missing_adrs:
        problems.append("ADRs citados no código/README mas AUSENTES no Word: "
                        + ", ".join(missing_adrs))

    # (2) Arquivos
    files = find_project_files()
    missing_files = sorted(f for f in files
                           if f not in txt and f.rsplit('.', 1)[0] not in txt)
    if missing_files:
        problems.append("Arquivos do projeto AUSENTES no Word: " + ", ".join(missing_files))
    ...
    print(f"OK — documentação em dia: {len(adrs)} ADRs e {len(files)} arquivos "
