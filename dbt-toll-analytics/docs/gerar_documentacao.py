...
r = p.add_run("dbt-toll-analytics")
r.bold = True
r.font.size = Pt(34)
r.font.color.rgb = AZUL
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = p.add_run("Documentação Didática Completa do Projeto")
r.font.size = Pt(16)
r.font.color.rgb = CINZA
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = p.add_run(
    "Engenharia de Dados com dbt + DuckDB — Analytics e Auditoria de Vale-Pedágio"
)
r.italic = True
r.font.size = Pt(12)
r.font.color.rgb = CINZA
...
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = p.add_run(
    "Documento gerado automaticamente a partir do projeto real (validado com dbt build verde)."
)
r.italic = True
r.font.size = Pt(9)
r.font.color.rgb = CINZA
...
h("Sumário", 1)
para(
    "Para atualizar os números de página: clique com o botão direito no sumário "
...
t2 = OxmlElement("w:t")
t2.text = "  [O sumário aparece aqui depois de abrir no Word e atualizar o campo — botão direito > Atualizar campo]"
...
para(
    "Este projeto é um “produto de dados” completo de auditoria e analytics de "
...
para(
    "Tudo roda 100% no seu computador, de graça, usando um banco de dados chamado "
...
table(
    ...
    [
        [
            "TARIFA_DIVERGENTE",
            "Valor cobrado diferente do esperado para aquela praça/veículo/data",
            "T0007, T0022, T0023",
        ],
        [
            "VALOR_INVALIDO",
            "Cobrança com valor nulo ou zero",
            "T0010 (zero), T0026 (nulo)",
        ],
        [
            "COBRANCA_EM_FALHA",
            "Transação marcada como FAILED/REVERSED mas com valor cobrado",
            "T0009 (FAILED), T0018 (REVERSED)",
        ],
        [
            "POSSIVEL_DUPLICIDADE",
            "Mesmo veículo na mesma praça em poucos minutos (cobrança repetida)",
            "T0015 + T0016 (3 min)",
        ],
    ],
    widths=[1.9, 3.1, 1.8],
...
para(
    "Resultado validado: o pipeline encontra exatamente 9 transações suspeitas "
...
para(
    "dbt (data build tool) é a ferramenta padrão de mercado para a camada de "
...
bullet(
    "cada transformação é um arquivo .sql pequeno que referencia outro — o dbt "
    "monta sozinho a ordem de execução (o DAG).",
    "Modularidade: ",
)
bullet(
    "você declara regras (“essa coluna não pode ser nula”, “essa chave é "
    "única”) e o dbt valida automaticamente.",
    "Testes de dados: ",
)
bullet(
    "o dbt gera um site navegável com a descrição de cada tabela e o lineage "
    "(o mapa de dependências de ponta a ponta).",
    "Documentação + lineage: ",
)
bullet(
    "o mesmo SQL que roda localmente no DuckDB roda no Databricks, "
    "Snowflake, BigQuery, etc. — basta trocar a conexão.",
    "Portabilidade: ",
)
para(
    "Em resumo: dbt transforma “escrever SQL” em “fazer engenharia de dados”. "
...
para(
    "DuckDB é um banco analítico que roda em um único arquivo local, sem servidor, "
...
table(
    ...
    [
        ...
        [
            "Git / GitHub",
            "Versionamento e CI (GitHub Actions roda o pipeline em cada PR).",
        ],
        ["SQLFluff", "Linter que verifica o estilo/qualidade do SQL."],
        [
            "Elementary (dbt package)",
            "Observabilidade de dados: detecção de anomalia + tabelas de monitoramento.",
        ],
...
para(
    "O projeto organiza os dados em três camadas de qualidade crescente — o padrão "
...
table(
    ...
    [
        [
            "BRONZE (cru)",
            "seeds/",
            "Dados crus, exatamente como chegaram — inclusive os erros propositais.",
        ],
        [
            "SILVER (limpo)",
            "models/staging/",
            "Tipagem, limpeza, deduplicação e mascaramento de PII. 1 modelo por fonte. Sem juntar fontes.",
        ],
        [
            "GOLD (consumo)",
            "models/intermediate/ + models/marts/",
            "Enriquecimento (joins + regra de negócio) e o modelo dimensional + auditoria.",
        ],
    ],
    widths=[1.5, 2.1, 3.2],
)
...
code(
    "seeds (bronze)  ->  staging (silver)  ->  intermediate  ->  marts (gold)\n"
    "raw_*.csv            stg_*.sql            int_*.sql         dim_*/fct_*/agg_*/audit_*"
)
para(
    "A regra de ouro: a camada silver só “arruma a casa” (não junta fontes); "
...
para(
    "Construímos respeitando o DAG (de baixo para cima): primeiro a base, depois o que "
    "depende dela, validando a cada bloco. Esta foi a ordem exata seguida:"
)
passos = [
    (
        "Passo 1 — Configuração",
        "Ajustamos packages.yml (bibliotecas dbt_utils e dbt_expectations) e requirements.txt (dbt-duckdb e sqlfluff). É a fundação: define quais ferramentas e bibliotecas o projeto usa.",
    ),
    (
        "Passo 2 — Seeds (bronze)",
        "Tiramos a tarifa fixa da tabela de praças e criamos raw_fare_schedule.csv, a tabela de tarifa COM VIGÊNCIA (com a mudança de preço da praça P003). Isso habilita a técnica point-in-time.",
    ),
    (
        "Passo 3 — Staging (silver)",
        "Ajustamos stg_toll_plazas, criamos stg_fare_schedule e o principal: stg_toll_transactions (deduplicação + tipagem + chave técnica). Documentamos e testamos tudo em _staging.yml.",
    ),
    (
        "Passo 4 — Snapshot",
        "Criamos snap_toll_plazas para demonstrar a captura de histórico (SCD2) nativa do dbt.",
    ),
    (
        "Passo 5 — Intermediate",
        "Criamos int_transactions_enriched: o coração do projeto, onde acontece o join point-in-time da tarifa e a detecção de duplicidade na janela de tempo.",
    ),
    (
        "Passo 6 — Macros",
        "Criamos cents_to_brl (converte centavos para reais), audit_flag (a regra de auditoria) e log_run_results (observabilidade).",
    ),
    (
        "Passo 7 — Marts (gold)",
        "Criamos as dimensões (dim_date, dim_plaza, dim_vehicle), o fato incremental (fct_toll_transactions), a agregação de receita e o produto final audit_suspect_transactions. Aplicamos contratos de dados e unit tests.",
    ),
    (
        "Passo 8 — Testes",
        "Criamos um teste genérico customizado (not_charged_when_failed) e um teste singular de unicidade no fato.",
    ),
    (
        "Passo 9 e 10 — Exposures e CI",
        "Declaramos o consumidor downstream (relatório aos gestores) no lineage, e configuramos o CI (GitHub Actions) + o linter SQLFluff.",
    ),
    (
        "Passo 11 — README com ADRs",
        "Reescrevemos o README documentando as decisões de arquitetura (ADRs).",
    ),
    (
        "Passo 12 — Validação",
        "Criamos o ambiente Python, instalamos as dependências e rodamos dbt build — resultado verde (PASS=92, WARN=1 intencional, ERROR=0).",
    ),
    (
        "Passo 13 — Hardening sênior",
        "Após um code review crítico, corrigimos 3 bugs reais (fan-out no join, late-arriving data, dim_date fixa) e adicionamos diferenciais (Semantic Layer, dev/prod). Ver seções 9.1 e 9.2.",
    ),
]
...
para(
    "Esta é a parte mais detalhada: cada arquivo do projeto, agrupado por pasta, com a "
...
def arquivo(nome, o_que, porque, logica=None):
    ...
    q = doc.add_paragraph()
    q.add_run("O que é / para que serve: ").bold = True
    q.add_run(o_que)
    q.paragraph_format.space_after = Pt(2)
    w = doc.add_paragraph()
    w.add_run("Por que existe: ").bold = True
    w.add_run(porque)
    w.paragraph_format.space_after = Pt(2)
    if logica:
        lg = doc.add_paragraph()
        lg.add_run("Lógica interna: ").bold = True
        lg.add_run(logica)
        lg.paragraph_format.space_after = Pt(4)
...
para(
    "O bronze NÃO é mais seed (ADR-28): o projeto ingestion-toll-analytics usa o "
...
arquivo(
    "toll_ingestion.py (dlt) + _sources.yml",
...
arquivo(
    "stg_fare_schedule.sql (+ teste mutually_exclusive_ranges)",
...
arquivo(
    "int_duplicate_flags.sql (EPHEMERAL)",
...
arquivo(
    "agg_daily_revenue_by_plaza.sql",
...
arquivo(
    "audit_suspect_transactions.sql",
...
arquivo(
    "py_plaza_audit_stats.py (PYTHON MODEL)",
...
arquivo(
    "rpt_plaza_revenue_v1.sql / rpt_plaza_revenue_v2.sql (VERSIONED)",
...
arquivo(
    "tests/generic/test_not_charged_when_failed.sql",
...
arquivo(
    "tests/assert_unique_transaction_in_fct.sql",
...
para(
    "O problema: o preço do pedágio muda no tempo. Se eu comparar uma transação ANTIGA "
...
para(
    "A solução: juntar a transação à tabela de tarifa pela DATA DO EVENTO "
    "(event_date BETWEEN valid_from AND valid_to), pegando a tarifa que estava vigente "
    "naquele dia específico."
)
para(
    "Exemplo real nos dados (praça P003, carreta categoria 9, multiplicador 4.5x; a "
    "tarifa muda de 990 para 1050 em 2026-05-03):",
    bold=True,
)
table(
...
para(
    "Repare: a MESMA transação (valor 4455) é correta em 01/05 e divergente em 03/05, "
...
para(
    "Rodamos o pipeline completo de verdade. Saída do dbt build do CI de PR "
...
bullet(
    "124 verificações passaram (testes de dados + unit tests + ranges + audit_flag + modelos do Elementary). A detecção de anomalia roda separada: dbt test --select tag:observability."
)
bullet(
    "1 WARN é INTENCIONAL: o teste not_charged_when_failed pega as cobranças em "
    "FAILED/REVERSED que existem de propósito — a auditoria as sinaliza, então é "
    "alerta monitorável (warn), não falha de build."
)
bullet("0 erros.")
bullet(
    "Idempotência confirmada: rodar duas vezes mantém o fato com 26 linhas (o "
    "incremental não duplica)."
)
para(
    "Distribuição final da auditoria: 3 TARIFA_DIVERGENTE, 2 VALOR_INVALIDO, "
...
para(
    "ADR = Architecture Decision Record. Registrar o julgamento por trás das decisões é "
    "o que mais sinaliza senioridade."
)
adrs = [
    (
        "ADR-1 — Tarifa no schedule, não na praça",
        "Preço tem história; modelar com vigência permite o join point-in-time e evita falso positivo.",
    ),
    (
        "ADR-2 — Histórico de tarifa via seed, não snapshot",
        "Snapshot só constrói história ao longo de várias execuções; para um portfólio reprodutível, precisamos da história já na 1a execução.",
    ),
    (
        "ADR-3 — Valor inválido: flag, não delete",
        "Em auditoria, a regra é sinalizar e manter (rastreabilidade), nunca apagar o dado.",
    ),
    (
        "ADR-4 — Fato incremental com unique_key",
        "Em escala não reprocessa o histórico; unique_key garante idempotência.",
    ),
    (
        "ADR-5 — Regra de auditoria isolada na macro",
        "Regra de negócio testável (unit tests) e reutilizável, desacoplada dos dados reais.",
    ),
    (
        "ADR-6 — Severidade warn para cobrança em falha",
        "São exatamente o que a auditoria existe para achar; quebrar o build seria contraditório. error fica para invariantes que nunca podem ocorrer.",
    ),
    (
        "ADR-7 — DuckDB local, portável para Databricks",
        "Loop de feedback local instantâneo e sem custo; os modelos SQL são os mesmos na nuvem.",
    ),
    (
        "ADR-8 — Incremental com janela de lookback",
        "Reprocessa os últimos N dias para capturar transações atrasadas (late-arriving); o unique_key deduplica. Evita o bug clássico de descartar dados que chegam fora de ordem.",
    ),
    (
        "ADR-9 — Guarda contra fan-out no join temporal",
        "Teste mutually_exclusive_ranges impede vigências sobrepostas por praça, que duplicariam transações silenciosamente.",
    ),
    (
        "ADR-10 — dim_date com faixa derivada dos dados",
        "Em vez de fixa em 2026, deriva do min/max real — robusto a datas fora do ano.",
    ),
    (
        "ADR-11 — Semantic Layer em vez de só agregações fixas",
        "Métricas definidas uma vez e consultáveis por qualquer dimensão; garante consistência da métrica.",
    ),
    (
        "ADR-12 — audit_flag materializada no fato",
        "Habilita métricas de suspeita no Semantic Layer sem duplicar regra (a lógica vive na macro — DRY).",
    ),
    (
        "ADR-13 — sources + freshness sobre os seeds",
        "Declara o contrato de ingestão (SLA de atraso) sem perder o bronze reprodutível offline; em prod, seeds viram tabelas de ingestão e o staging passa a usar source().",
    ),
    (
        "ADR-14 — Observabilidade de dados com Elementary",
        "Pacote elementary + teste de anomalia de volume; tabelas de monitoramento materializadas no warehouse. Salto de 'tenho metadados' para 'monitoro dados'. Ressalvas honestas no DuckDB: anomaly detection precisa de histórico (fica em warn); relatório HTML do edr é melhor em warehouses cloud.",
    ),
    (
        "ADR-15 — Dinheiro agregado em centavos inteiros",
        "Agregações monetárias somam amount_cents (inteiro, exato) e convertem para BRL só no final; nunca somam reais já arredondados. Evita acúmulo de erro de arredondamento/float — padrão em dado financeiro. Vale para o agg e para o Semantic Layer.",
    ),
    (
        "ADR-16 — Testes genéricos no formato arguments:",
        "Todos os testes passaram a aninhar parâmetros sob 'arguments:', eliminando os deprecation warnings do dbt 1.11+ (à prova do dbt 2.0). dbt parse roda sem nenhum aviso.",
    ),
    (
        "ADR-17 — Observabilidade como job agendado (fora do PR)",
        "O teste de anomalia ganhou a tag 'observability'; o CI de PR roda 'dbt build --exclude tag:observability' (determinístico, PASS=124) e um workflow agendado roda a detecção de anomalia. Motivo: anomalia precisa de histórico acumulado e o autoupload do Elementary no DuckDB dispara erro de commit. Monitoramento contínuo não é gate de merge.",
    ),
    (
        "ADR-18 — Groups + access (governança de modelos)",
        "Cada model pertence a um group (staging/intermediate/marts) com dono, e tem nível de access: o interno fica 'protected' (só o package referencia) e os marts ficam 'public' (camada consumível por BI/Semantic Layer/exposures). Documenta as fronteiras de consumo. 'private' não se aplica porque os refs cruzam grupos.",
    ),
    (
        "ADR-19 — Constraints no warehouse (PK/CHECK via contract)",
        "Com contract enforced, declaramos constraints (primary_key, not_null, check) que viram DDL REAL na CREATE TABLE — o banco passa a garantir a invariante, não só o teste dbt. PK no grão do fato e das dims; CHECK em dim_date (mês 1..12, dia da semana 0..6).",
    ),
    (
        "ADR-20 — Model ephemeral (int_duplicate_flags)",
        "A detecção de duplicidade virou um model EPHEMERAL: não materializa objeto no banco — o dbt inlina como CTE em quem der ref(). Separa 'achar duplicata' de 'enriquecer'. Uso clássico de ephemeral: passo lógico reutilizável e barato que não precisa ser consultado isoladamente.",
    ),
    (
        "ADR-21 — Python model (py_plaza_audit_stats)",
        "Um model em PYTHON (dbt-duckdb) calcula estatística de auditoria por praça (taxa de suspeita e z-score entre praças) em pandas. O dbt orquestra/testa/versiona Python igual a SQL; usamos Python onde é natural (estatística), não por moda.",
    ),
    (
        "ADR-22 — Versioned model + deprecation_date",
        "rpt_plaza_revenue é VERSIONADO: a v2 (latest) adiciona ticket médio; a v1 fica deprecada até 2026-12-31. Quebra de contrato via VERSÃO, não in-place — os consumidores migram no seu ritmo. ref() resolve para a latest_version.",
    ),
    (
        "ADR-23 — dbt_project_evaluator como warn",
        "Adicionamos o pacote que audita o PRÓPRIO projeto contra best practices (naming, fanout, undocumented, modelos públicos sem contract...). As descobertas ficam como 'warn' — recomendações de melhoria contínua monitoráveis, não gate de merge.",
    ),
    (
        "ADR-24 — Materializações/grants só no target prod (Databricks)",
        "materialized_view, estratégia incremental microbatch e grants são features do warehouse de produção (Databricks/Delta), não do DuckDB single-node de dev. Em vez de falsear no dev, ficam documentadas/config no target prod. No DuckDB rodam as materializações que ele suporta (view/table/incremental/ephemeral).",
    ),
    (
        "ADR-25 — Docs blocks + persist_docs",
        "Descrições reutilizáveis viram docs blocks ({% docs %} em models/docs.md, referenciados com doc()), e +persist_docs empurra as descrições para COMMENTs no banco. Documentação versionada, sem duplicação, que vive junto do dado. Validado: o COMMENT do audit_flag no DuckDB traz o docs block renderizado.",
    ),
    (
        "ADR-26 — Saved queries + exports (Semantic Layer)",
        "Uma saved query (revenue_daily) agrupa métricas + recorte + export no Semantic Layer — consulta governada e reaproveitável, em vez de cada dashboard copiar SQL. O export materializa a métrica numa tabela (dbt Cloud / mf export); em core a definição é versionada e validada (mf validate-configs).",
    ),
    (
        "ADR-27 — Slim CI (state:modified+ --defer)",
        "Um workflow de PR (dbt_slim_ci.yml) constrói só o que mudou + downstream, deferindo o resto a um baseline (manifest da branch base). Reconstruir tudo a cada PR é caro; o Slim CI roda em segundos quando pouca coisa muda. Validado: dbt ls --select state:modified+ seleciona exatamente o model alterado. Em prod, o --defer aponta para o warehouse.",
    ),
    (
        "ADR-28 — Ingestão (EL) com dlt; bronze deixa de ser seed",
        "Realiza o ADR-13: um pipeline dlt (projeto ingestion-toll-analytics) lê os arquivos de landing (CSV) e carrega no schema 'landing' do DuckDB (merge/replace, metadados de carga, '' -> NULL). O staging passa a consumir via source('toll_raw', ...) em vez de seeds; os seeds foram removidos. Continua reprodutível offline (o dlt lê CSVs commitados, não uma API). O Airflow roda a ingestão antes do transform; o CI também (dlt -> dbt build).",
    ),
]
...
para(
    "Depois de um code review crítico (o tipo de olhar que separa pleno de sênior), "
...
table(
    ...
    [
        [
            "Fan-out no join point-in-time",
            "Se duas vigências de tarifa se sobrepusessem, a transação duplicaria silenciosamente.",
            "Teste mutually_exclusive_ranges (ADR-9).",
        ],
        [
            "Late-arriving data no incremental",
            "Filtro '> max(data)' descartava PARA SEMPRE transações que chegam atrasadas.",
            "Janela de lookback de N dias + merge (ADR-8).",
        ],
        [
            "dim_date travada em 2026",
            "Qualquer transação fora de 2026 quebraria o teste de relationships.",
            "Faixa derivada do min/max real (ADR-10).",
        ],
        [
            "Dinheiro somado em float",
            "Somar reais já arredondados (amount_brl) acumula erro de arredondamento.",
            "Somar centavos inteiros e converter no final (ADR-15).",
        ],
    ],
    widths=[2.0, 3.0, 1.8],
)
para(
    "Nota de processo: um segundo code review apontou alegações que JÁ estavam corrigidas "
    ...
    "aceitar ou descartar, é parte da postura sênior.",
    italic=True,
)
...
para(
    "Adicionamos o Semantic Layer do dbt — a feature mais 'sênior 2025/2026'. Em vez "
...
para(
    "Limitações conhecidas e roadmap (documentar o próprio risco é o movimento que mais "
    "impressiona um recrutador técnico):",
    bold=True,
)
bullet("Late-arriving além da janela de N dias: mitigado e documentado (ADR-8).")
bullet(
    "sources + freshness: IMPLEMENTADO (ADR-13) — contrato de ingestão declarado, com SLA de atraso; validado com dbt source freshness."
)
bullet(
    "Observabilidade avançada (Elementary): IMPLEMENTADO (ADR-14) — pacote + detecção de anomalia + tabelas de monitoramento no warehouse. Ressalva honesta: relatório HTML do edr é melhor em warehouses cloud que no DuckDB."
)
bullet(
    "Orquestração (Airflow + Cosmos): a camada de agendamento/observabilidade por cima fecharia o fim-a-fim. Próximo passo."
)
bullet(
    "Docs hospedados: publicar o dbt docs no GitHub Pages para lineage navegável linkável no LinkedIn."
)
...
glos = [
    (
        "ELT",
        "Extract, Load, Transform — primeiro carrega o dado cru, depois transforma dentro do banco. O dbt cuida do T.",
    ),
    (
        "Medallion",
        "Arquitetura em 3 camadas: bronze (cru), silver (limpo), gold (consumo).",
    ),
    ("Model", "Um arquivo .sql com um SELECT; o dbt o transforma em view ou tabela."),
    (
        "ref()",
        "Função do dbt que referencia outro modelo e monta o DAG (ordem de execução).",
    ),
    (
        "DAG / lineage",
        "O grafo de dependências entre os modelos, gerado automaticamente.",
    ),
    (
        "Materialização",
        "Como o modelo vira objeto: view, table, incremental, ephemeral.",
    ),
    ...
    (
        "Contract",
        "Contrato de dados que trava colunas e tipos; quebra o build se mudar.",
    ),
...
para(
    "Abaixo, falas prontas (técnica -> o que dizer). Use no resumo, na descrição da "
...
table(
    ...
    [
        [
            "Point-in-time (SCD2 via schedule)",
            "“Modelei tarifa com vigência e join point-in-time para evitar falso positivo em transações históricas.”",
        ],
        [
            "Model incremental",
            "“Materializei o fato como incremental com unique_key para não reprocessar histórico em escala.”",
        ],
        [
            "Model contracts",
            "“Apliquei contratos de dados nos marts — o build quebra se o schema mudar sem querer.”",
        ],
        [
            "Unit tests (dbt 1.8)",
            "“Cobri a lógica de auditoria com unit tests e inputs mockados, além dos testes de dados.”",
        ],
        [
            "Observabilidade",
            "“Tenho observabilidade: materializo falhas de teste e gravo metadados de execução.”",
        ],
        [
            "CI + SQLFluff",
            "“PR roda dbt build + lint; em projeto grande, Slim CI com state:modified e deferral.”",
        ],
        [
            "Masking de PII",
            "“Mascaro PII (placa) já na camada silver, por LGPD e menor exposição.”",
        ],
        [
            "Exposures",
            "“Declaro o consumidor downstream (relatório de auditoria) no lineage.”",
        ],
        [
            "Medallion + dbt",
            "“Estruturei o pipeline em camadas Medallion com dbt: bronze, silver e gold, com testes em cada camada.”",
        ],
        [
            "Lookback (late-arriving)",
            "“Meu fato incremental tem janela de lookback com merge para não perder transações que chegam atrasadas — e documentei o trade-off.”",
        ],
        [
            "Guarda contra fan-out",
            "“Meu join temporal poderia gerar fan-out se as vigências se sobrepusessem, então tranquei isso com um teste de ranges mutuamente exclusivos.”",
        ],
        [
            "Semantic Layer (MetricFlow)",
            "“Defini métricas no Semantic Layer do dbt (revenue, suspect_rate, vazamento de receita) — uma definição governada, consultável por qualquer dimensão.”",
        ],
        [
            "Sources + freshness",
            "“Declarei o contrato de ingestão com sources e source freshness (SLA de atraso), mostrando que penso o pipeline desde a entrada do dado, não só a transformação.”",
        ],
        [
            "Observabilidade (Elementary)",
            "“Integrei o Elementary para observabilidade de dados — detecção de anomalia e tabelas de monitoramento no warehouse — e conheço os trade-offs de rodá-lo em DuckDB vs warehouse cloud.”",
        ],
        [
            "Dinheiro em centavos inteiros",
            "“Agrego dinheiro em centavos inteiros e só converto para reais na exibição, pra não acumular erro de arredondamento/float — disciplina de dado financeiro.”",
        ],
        [
            "Observabilidade agendada",
            "“Separei a detecção de anomalia num job agendado (tag observability), fora do CI de PR — monitoramento contínuo precisa de histórico, não é gate de merge.”",
        ],
        [
            "Reconhecer o próprio risco",
            "“Documentei as limitações que eu mesmo encontrei (late-arriving, sources/freshness, observabilidade) num roadmap — antes de alguém perguntar.”",
        ],
    ],
    widths=[2.3, 4.5],
)
...
para(
    "“Engenheiro de Dados — construo pipelines de transformação com dbt (camadas "
...
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
r = p.add_run(
    "Fim do documento — dbt-toll-analytics. Pipeline validado com dbt build verde."
)
r.italic = True
r.font.size = Pt(9)
r.font.color.rgb = CINZA
